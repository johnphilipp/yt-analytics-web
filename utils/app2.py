...
def display_video_data(video_id, video_data):
    col1, col2, col3 = st.sidebar.columns([0.2, 1.2, 1.1])
    col2.image(video_data["thumbnail_url"], width=140)
    # col3.text(video_data["channel"])
    # col3.text(video_data["date_released"])
    col3.text(human_format(int(video_data["num_comments"])) + " comments")
    col3.text(human_format(int(video_data["num_views"])) + " views")
