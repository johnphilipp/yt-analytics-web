...
def display_video_data(video_id, video_meta_data):
    col1, col2, col3 = st.sidebar.columns([0.2, 1.2, 1.1])
    col2.image(video_meta_data["thumbnail_url"], width=140)
    # col3.text(video_data["channel"])
    # col3.text(video_data["date_released"])
    col3.text(human_format(int(video_meta_data["comment_count"])) + " comments")
    col3.text(human_format(int(video_meta_data["view_count"])) + " views")
